...
{
    public class EquipmentAccess:UserAccess
    {

        public EquipmentAccess(SharedContext context, System.Security.Principal.IPrincipal LoggedInUser) : base(context)
        {
            Initialized = Init(LoggedInUser);
        }
        public EquipmentAccess(SharedContext context) : base(context)
        {

        }

        public EquipmentAccess(SharedContext context, int _UserId) : base(context)
        {
            Initialized = Init(_UserId);
        }

        public ResultMessage AddUserToEquipment(int EquipmentId, int _UserId)
        {
            if (!Initialized)
            ...
            var entities = _domainContext.USER_EQUIPMENT_RELATION.Where(m => m.UserId == _UserId && m.EquipmentId == EquipmentId && m.RecordStatus == (int)RecordStatus.Available);
            if (entities.Count() > 0)
                return new ResultMessage { Id = 0, LastMessage = "Operation Failed! User is already exist in this dealer!", OperationSucceed = false };
            var entity = new USER_EQUIPMENT_RELATION { EquipmentId = EquipmentId, UserId = _UserId, AddedByUserId = UserId, AddedDate = DateTime.Now.ToLocalTime() };
            _domainContext.USER_EQUIPMENT_RELATION.Add(entity);
            try
            {
        ...
        public List<ResultMessage> AddUserToEquipment(int EquipmentId, List<int> _UserIds)
        {
            ...
            var entityRange = new List<USER_EQUIPMENT_RELATION>();
            foreach (var _UserId in _UserIds.Distinct())
            {
                var entities = _domainContext.USER_EQUIPMENT_RELATION.Where(m => m.UserId == _UserId && m.EquipmentId == EquipmentId && m.RecordStatus == (int)RecordStatus.Available);
                if (entities.Count() > 0)
                {
                    result.Add(new ResultMessage { Id = 0, LastMessage = "Operation Failed! User is already exist for this equipment!", OperationSucceed = false });
                    continue;
                }
                entityRange.Add(new USER_EQUIPMENT_RELATION { EquipmentId = EquipmentId, UserId = _UserId, AddedByUserId = UserId, AddedDate = DateTime.Now.ToLocalTime() });
            }
            var addedEntities = _domainContext.USER_EQUIPMENT_RELATION.AddRange(entityRange);
            try
            {
                _domainContext.SaveChanges();
            }
        ...
        public ResultMessage RemoveUserFromEquipment(int EquipmentId, int _UserId)
        {
            if (!Initialized)
                return new ResultMessage { Id = 0, LastMessage = "Operation Failed! Your user account seems not exist!", OperationSucceed = false, ActionLog = "Operation Failed because this user may have not setup correctly. There is no user in the USER_TABLE associated with this user AspNetId, so initialization failed!" };
        ...
        public ResultMessage RemoveUserFromEquipment(int EquipmentId, List<int> _UserIds)
        {
            if (!Initialized)
            {
                return new ResultMessage { Id = 0, LastMessage = "Operation Failed! Your user account seems not exist!", OperationSucceed = false, ActionLog = "Operation Failed because this user may have not setup correctly. There is no user in the USER_TABLE associated with this user AspNetId, so initialization failed!" };
            ...
            foreach (var _UserId in _UserIds)
            {
                ...
                if (entities.Count() == 0)
                {
                    //ResultMessage { Id = 0, LastMessage = "Operation Failed! Equipment is not found or has already been removed!", OperationSucceed = false });
                    continue;
                }
                foreach (var entity in entities)
                {
        ...
        public bool checkUserRelation(int _userId, int _equipmentId)
        {
            return _domainContext.USER_EQUIPMENT_RELATION.Where(m => m.EquipmentId == _equipmentId && m.UserId == _userId && m.RecordStatus == (int)RecordStatus.Available).Count() > 0;
        }
